...
class CNNCTCAdapter(ModelAdapter):
    def __init__(self, model_id: str = "cnn_ctc", device: str = "cpu", simulate: bool = True) -> None:
    ...
    def load(self) -> None:
        ...
        self.backend_detail = "cnn_ctc proxy / simulated"
        if self.requested_simulate:
            self.runtime_note = "当前使用 CNN-CTC 代理基线，用于轻量对照。"
        else:
            self.runtime_note = "当前项目尚未接入真实 CNN-CTC 模型，已自动回退为代理基线进行对照。"
