...
def render_metric_chart(container, frame: pd.DataFrame, spec: dict[str, str]) -> None:
    ...
    chart_data[label] = pd.to_numeric(chart_data[label], errors="coerce").fillna(0.0)
    chart_data["模型"] = chart_data["模型"].astype(str)
    chart_data = chart_data.sort_values(label, ascending=metric_key in LOWER_IS_BETTER).reset_index(drop=True)

    with container:
        st.markdown(
            f'<div class="chart-card"><h4 class="chart-title">{html.escape(label)}</h4><p class="chart-hint">{html.escape(hint)}</p></div>',
        ...
        chart = (
            ...
            .encode(
                x=alt.X("模型:N", sort=chart_data["模型"].tolist(), axis=alt.Axis(labelAngle=0, title=None)),
                y=alt.Y(f"{label}:Q", title=None),
                color=alt.value(color),
                tooltip=[
                    alt.Tooltip("模型:N"),
                    alt.Tooltip(f"{label}:Q", format=fmt),
                ],
            )
...
def render_chart_section() -> None:
    section_header("05 / Charts", "可视化图表多模型对比", "为每一个核心指标绘制单独图表，分别观察精度、性能和满意度维度的差异。")
    ...
    try:
        ...
        performance_specs = [
            ...
            if spec["key"] in {"avg_latency_ms", "p95_latency_ms", "avg_upl_ms", "avg_rtf", "throughput", "load_time_ms", "cpu_pct", "mem_mb"}
        ]
